    ...
    public class Utils
    {
        ...
        public static object MyDLookup(String pTarget, String pDomain, String pCriteria, String pAggregate)
        {
            lock (thisLock)
            {
                ...
                if ((pDomain.Length > 7) && (pDomain.Substring(0, 6) == "SELECT"))
        ...
        public static int AddStore(String store,
                    String address1,
                    String address2,
                    String city,
                    String state,
            ...
            int storeID = 0;
            ExecuteNonQuery("INSERT INTO tStore(StoreNumber, Store, Address1, Address2, City, State, Zip, ManagerID) " +
                            "VALUES (" + Config.SingleQuoteMe(store)
                            + ", " + Config.SingleQuoteMe(address1)
                            + ", " + Config.SingleQuoteMe(address2)
                            + ", " + Config.SingleQuoteMe(city)
                            + ", " + Config.SingleQuoteMe(state)
                            + ", " + Config.SingleQuoteMe(zip)
                            + ", " + Convert.ToString(managerID)
                            + ", " + Config.SingleQuoteMe(storeNumber)
                            + ", " + Convert.ToString(isVirtual)
                            + ", " + Convert.ToString(countryID)
                            + ")",
                            CommandType.Text,
                            null,
                            null);
        ...
        /// <summary>
        /// Read records from tCountry and add to a list box
        /// </summary>
        public static void LoadCountryListBox(System.Windows.Forms.ListBox lbCountry)
        {
            try
            {
                Utils.CheckConnection();
                LoadListBoxFromQuery(Config.myConnection,
                                     "Select CountryID, Country FROM tCountry ORDER BY Country",
                                     lbCountry);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Read records from tEmpl and add to a list box
        /// </summary>
        public static void LoadEmployeeListBox(System.Windows.Forms.ListBox lbEmpl)
        {
            try
            {
                Utils.CheckConnection();
                LoadListBoxFromQuery(Config.myConnection,
                                     "Select EmplID, LTRIM(RTRIM(Empl)) + " + 
                                                     Config.SingleQuoteMe(" ") +
                                                     " + LTRIM(RTRIM(LastName)) + " +
                                                     Config.SingleQuoteMe(" ") +
                                                     " + LTRIM(RTRIM(FirstName))  " + 
                                                     " FROM tEmpl ORDER BY LastName, FirstName",
                                     lbEmpl);
            }
            catch (Exception ex)
            {
        ...
        /// <summary>
        /// Read from a cursor into a list box
        /// </summary>
        /// <param name="conn">The open connection to the database</param>
        /// <param name="query">The query to use to populate the ListBox</param>
        /// <param name="lb">The ListBox</param>
        /// <returns></returns>
        public static int LoadListBoxFromQuery(SqlConnection conn, String query, ListBox lb)
        {
            int count = 0;
            try
            {
                ...
                if (reader.HasRows)
                {
                    // Step through all the records in the cursor, adding to the List Box
                    while (reader.Read())
                    {
                        things.Add(new Thing(reader.GetInt32(0), reader.GetString(1)));
                    }
                }
                else
                {
                    ...
                    throw new Exception("LoadListBoxFromQuery(): No items to read.");
                }
                //lb.DataSource = things;
                lb.Items.AddRange(things.ToArray());
            ...
            {
                Console.WriteLine("LoadListBoxFromQuery: " + ex.Message);
            }
            return count;
        }
    }
    // Used in LoadListBoxFromQuery() method
    class Thing
    {
        int key;
        String description;
        public Thing(int key, String description)
        {
            this.key = key;
            this.description = description;
        }
        public override String ToString() { return description; }   // override is crucial here, else the ToString from the base class will be used
    };
